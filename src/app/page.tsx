...
export default function Home() {
  ...
  // Category totals derived from expenses list
      ...
      <div className="mx-auto max-w-2xl space-y-8">
        ...
        {reviewing && result && (
          <div className="space-y-4">
            {preview && (
              <img src={preview} alt="Receipt preview" className="w-full rounded-xl object-contain shadow" style={{ maxHeight: 200 }} />
            )}
            <div className="rounded-xl border border-zinc-200 bg-white shadow-sm">
              ...
              <dl className="divide-y divide-zinc-100">
                {[
                  ...
                  { label: 'Total',    value: result.total != null ? fmt(result.total) : null },
                  { label: 'Tax',      value: result.tax  != null ? fmt(result.tax)   : null },
              ...
              <div className="flex gap-3 border-t border-zinc-100 px-4 py-3">
                <button onClick={handleSave} disabled={status === 'saving'} className="flex-1 rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:opacity-50">
                  {status === 'saving' ? 'Saving…' : 'Save'}
                </button>
                <button onClick={clearForm} disabled={status === 'saving'} className="flex-1 rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-600 transition hover:bg-zinc-50 disabled:opacity-50">
                  Discard
                </button>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {expenses.map(e => (
                    <tr key={e.id} className="hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3 text-zinc-500">{e.category ?? '—'}</td>
                      <td className="px-4 py-3 text-right font-medium text-zinc-900">{fmt(Number(e.total))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
